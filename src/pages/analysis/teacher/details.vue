<template>
  <div class="analysis-teacher-details absolute">
    <div class="main-view">
      <v-view-header title="师资分析详情">
        <div class="btn-list" slot="main">
          <el-button type="primary" @click="saveTeacherData" size="small">保存</el-button>
          <el-button @click="$router.push({ name: 'analysis-teacher'})" size="small">返回</el-button>
        </div>
      </v-view-header>
    ...
    async saveTeacherData() {
      ...
      if (d.code === 0) {
        ...
        if (this.$route.params.type === 0) {
          this.$router.push({ name: "analysis-teacher" });
        } else {
